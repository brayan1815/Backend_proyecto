...
public class ConsolasDAO {
    ...
    public List<ConsolaDTO> getAllConPrecio() {//se crea el metodo getAll, esye metodo retorna una lista con todas las consolas de la
                                   //base de datos
        
        List<ConsolaDTO> consolasDTO = new ArrayList<>();//se crea la lista que almacenara las consolas
        ...
        try (Connection conn = ConexionBD.getConnection()) {//se abre la conexion a la base de datos
            String sql = "SELECT \n" +
                        ...
                        "    consolas c\n" +
                        "INNER JOIN \n" +
                        "    tipos t ON c.id_tipo = t.id;";//se crea la consulta SQL
            ...
            while (rs.next()) {
                ...
                consolasDTO.add(consola);//el objeto consola se agrega a la lista
            }
        ...
        return consolasDTO;//se retorna la lista consolas
    }
    ...
    public Consola getByNumeroSerie(String numero) {//se crea el metodo getById, este metodo retornara una consola especifica segun su id
        Consola consola = null;//se crea la variable consola y se inicializa como null

        try (Connection conn = ConexionBD.getConnection()) {//se abre la conexion a la base de datos
            String sql = "SELECT * FROM consolas WHERE numero_serie=?";//se crea la consulta SQL
            PreparedStatement stmt = conn.prepareStatement(sql);//se prepara la consulra
            stmt.setString(1, numero);//se cambian los parametros de la consulta
